...
class Caixa240Registro5(CNAB240Registro5):
    ...
    qtd_registros_cobranca_simples = CNABFieldInteger(
        "", length=6, default="0", required=True
    )
    somatorio_valores_simples = CNABFieldDecimal(
        "", length=15, default="0", precision=2, required=True
    )
    qtd_registros_cobranca_caucionadas = CNABFieldInteger(
        "", length=6, default="0", required=True
    )
    somatorio_valores_caucionadas = CNABFieldDecimal(
        "", length=15, default="0", precision=2, required=True
    )
    qtd_registros_cobranca_descontada = CNABFieldInteger(
        "", length=6, default="0", required=True
    )
    somatorio_valores_descontada = CNABFieldDecimal(
        "", length=15, default="0", precision=2, required=True
    )
