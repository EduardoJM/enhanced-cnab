...
class DemoClass(RegistroBase):
    field_char = CNABFieldEnum(EnumA, "", length=6, default=EnumA.B, is_integer=False, required=True)
    field_int = CNABFieldEnum(EnumB, "", length=6, default=EnumB.AAAA, is_integer=True, required=True)
...
def test_instantiate_wrong_field_default_value():
    with pytest.raises(ValueError) as err:
        class NewClassExample(RegistroBase):
            field_char = CNABFieldEnum(EnumA, "", length=6, default=EnumB.AAAA, is_integer=False, required=True)
